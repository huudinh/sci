...
function showList(data) {
    for (let i = 0; i < data.length; i++) {
        let list = data[i];
        // console.log(list);
        if (list.c[0]) domain.name = list.c[0].v;
        else domain.name = '';
        if (list.c[1]) domain.dayReg = list.c[1].f;
        else domain.dayReg = '';
        if (list.c[2]) domain.dayEnd = list.c[2].f;
        else domain.dayEnd = '';
        if (list.c[3]) domain.dayCount = list.c[3].v;
        else domain.dayCount = '';
        if (list.c[4]) domain.brand = list.c[4].v;
        else domain.brand = '';
        if (list.c[5]) domain.ip = list.c[5].v;
        else domain.ip = '';
        if (list.c[6]) domain.person = list.c[6].v;
        else domain.person = '';
        if (list.c[7]) domain.address = list.c[7].v;
        else domain.address = '';
        if (list.c[8]) domain.phone = list.c[8].v;
        else domain.phone = '';
        if (list.c[9]) domain.email = list.c[9].v;
        else domain.email = '';
        if (list.c[10]) domain.trangql = list.c[10].v;
        else domain.trangql = '';
        if (list.c[11]) domain.note = list.c[11].v;
        else domain.note = '';
        if (list.c[13]) domain.ssl = list.c[13].v;
        else domain.ssl = '';

        // Xử lý null
        if (list.c[13] == null) domain.ssl = '';
...
function checkSSL(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if ((Number(list.c[13]) <= 10) && (list.c[13] != null)) {
            count++;
        }
    }
